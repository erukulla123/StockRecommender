#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""StockRecommender Web App — Flask backend (polling-based, no SSE)"""
...
# In-memory job store: job_id -> {status, pct, msg, result, error}
jobs = {}
...
def passes_filters(info, pe, params):
    ...
    if params.get("req_dividend"):
        if (info.get("dividendYield") or 0) <= 0:
    ...
    if params.get("req_analyst_up"):
        ...
        if target and price:
            if (target - price) / price * 100 < 10:
                return False, "Analyst upside < 10%"
        else:
            return False, "No analyst target"
    return True, ""
...
def run_screener_job(job_id, params):
    ...
    try:
        update(5, "Connecting to Yahoo Finance screener...")
        tickers = []
        try:
            ...
            update(10, f"Found {len(tickers)} candidates with P/E <= {params['max_pe']}. Fetching fundamentals...")
        except Exception as e:
            update(8, f"Trying broad screen...")
            try:
                from yfinance import EquityQuery
                q = EquityQuery('and', [
                    EquityQuery('eq',    ['region', 'us']),
                ...
                tickers = [q['symbol'] for q in result.get('quotes', []) if q.get('symbol')]
                update(10, f"Broad screen returned {len(tickers)} candidates...")
            except Exception as e2:
                jobs[job_id].update({"status": "error", "msg": f"Screener failed: {e2}"})
                return

        if not tickers:
            jobs[job_id].update({"status": "error", "msg": "No tickers returned from Yahoo Finance."})
            return
        ...
        for i, ticker in enumerate(tickers):
            ...
            if i % 10 == 0:
                update(pct, f"Analyzing {ticker}... ({i}/{total})")
            try:
        ...
        update(82, f"Found {passed} qualifying stocks. Generating AI analysis...")
        ...
        jobs[job_id].update({
            ...
            "pct": 100,
            "msg": "Done!",
            "result": {"stocks": top, "total_scanned": passed + failed, "passed": passed}
        })
...
def build_email_html(stocks, params):
    ...
    for s in stocks:
        ...
        cards += (
            ...
            f'<div style="font-size:0.75rem;color:#6b7280;margin-top:2px;">{s["company"]} &nbsp;&middot;&nbsp; {s["sector"]}</div></td>'
            f'<td align="right">'
            ...
            f'<div style="display:flex;justify-content:space-between;font-size:0.6rem;color:#9ca3af;margin-bottom:3px;"><span>Score</span><span style="color:{sc_col};font-weight:600;">{score}</span></div>'
...
def send_report_email(to_email, stocks, params):
    ...
    try:
        ...
        return False, "SMTP authentication failed. Check credentials in .env"
    except Exception as e:
        return False, str(e)
...
def start_screen():
    """Start a background screening job, return job_id immediately."""
    ...
    t = threading.Thread(target=run_screener_job, args=(job_id, params), daemon=True)
    t.start()
